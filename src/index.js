...
const sun = new SunAstroTimes(timeNow, plane.getLatitude(), plane.getLongitude(), plane.getAltitudeFeet());
...
function calculateProjectedLatLong(distanceToSunset, timeToPoint){
    ...
    newLongitude = plane.getLongitude() + deg.toDegrees(Math.atan2(Math.sin(directionRad) * Math.sin(distanceToSunset/3440) * Math.cos(latitudeRad),
                                    Math.cos(distanceToSunset/3440) - Math.sin(latitudeRad) * Math.sin(newLatitude)));
    
    
    newLatitude = deg.toDegrees(newLatitude);
    sun.reCalcTimes(timeToPoint, newLatitude, newLongitude, plane.getAltitudeFeet());
...
function checkForSunsetIntercept(){    
    let timeToPoint = 0;
    for(let i = 1; i <= plane.getSpeed()*12; i+=0.1){
        ...
        if(sun.getSunset() <= timeToPoint){
            ...
            console.log("Time to go is:     " + (timeToPoint - timeNow) / oneHour + "Hrs");
            break;
        }
    }
}
function checkForSunriseIntercept(){    
    let timeToPoint = 0;
    for(let i = plane.getSpeed()*14; i >= 400; i--){
        timeToPoint = i / plane.getSpeed() * oneHour + timeNow;
        calculateProjectedLatLong(i , timeToPoint);
        ...
        if(sun.getSunrise() >= timeToPoint){
            ...
            console.log("Distance to go is:     " + i +"NM");
            console.log("Time to go is:         " + (timeToPoint - timeNow) / oneHour + "Hrs");
            console.log(sun.getSunrise());
            console.log(timeToPoint);
            console.log(sun.getSunrise() >= timeToPoint);
            break;
        }   
